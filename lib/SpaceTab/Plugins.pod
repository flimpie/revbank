...
SpaceTab::Plugins - Plugin mechanism for SpaceTab

=head1 DESCRIPTION

SpaceTab itself consists of a simple command line interface and a really brain
dead shopping cart. All transactions, even deposits and withdrawals, are
handled by plugins.

Plugins are defined in the C<spacetab.plugins> file. Each plugin is a Perl
source file in the C<plugins> directory. Plugins are always iterated over in
the order they were defined in.

=head2 Methods

=head3 SpaceTab::Plugins::load

Reads the C<spacetab.plugins> file and load the plugins.

=head3 SpaceTab::Plugins->new

Returns a B<list> of fresh plugin instances.

=head3 SpaceTab::Plugins::register($package)

Registers a plugin.

=head3 SpaceTab::Plugins::call_hooks($hook, @arguments)
...
Whenever a command is given in the 'outer' loop of spacetab, the C<command>
...
The plugin declines handling of the given command, and spacetab should proceed
with the next one.
...
SpaceTab will either query the user again, or (if there is any remaining input
...
C<SpaceTab::Messages>. Such a hack might look like:

    undef &SpaceTab::Messages::hook_abort;
...
Several global utility functions are available. See L<SpaceTab::Global>
