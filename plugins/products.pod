...
products - SpaceTab plugin for selling products
...
This plugin turns products from a product list into SpaceTab commands,
...
Note: if a product id is the same as another SpaceTab command (e.g. a username),
the first plugin that accepts the command will "win"; the precedence order is
defined by the C<spacetab.plugins> configuration file. However, when a product
id appears multiple times within C<spacetab.products>, the I<last> one is used.
...
C<spacetab.products>. User accounts are liability accounts.)
...
The addon product must be specified in C<spacetab.products>; market products
