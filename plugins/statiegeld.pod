...
statiegeld - SpaceTab plugin for return deposits
...
In a typical setup, there would be a separate terminal where SpaceTab runs in
...
Alternatively, on a regular SpaceTab terminal, the C<deposit> command (which is
provided by the C<deposit> plugin) is extended to support product id's where
you would normally enter an amount.

=head2 Usage

If SpaceTab was run with the environment variable C<REVBANK_STATIEGELD> set to
...
The statiegeld plugin recognises products from C<spacetab.products> by matching
...
extensively in SpaceTab, it was useful to distinguish between the two features.
